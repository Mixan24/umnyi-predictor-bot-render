...
# ⚙️ Инициализация приложения Telegram
...
def calc_goal_probability(stats):
    try:
    ...
    while True:
        try:
            ...
            for match in matches:
                ...
                prob = calc_goal_probability(values)
                key = f"{home}-{away}"
...
# 🚀 Основной запуск (webhook-режим)
async def main():
    app.add_handler(CommandHandler("start", start))
    asyncio.create_task(analyze_live_matches())

    # Очистка старого webhook и установка нового
