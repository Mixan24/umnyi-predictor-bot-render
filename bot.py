...
# ⚽ Алгоритм оценки вероятности гола
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"👋 Привет, {first_name}!\n"
        f"Ты успешно подключён к системе ⚽ прогнозов.\n"
        f"Теперь я буду присылать уведомления, если вероятность гола > 80 %."
    )

    print(f"[✅] Новый пользователь: {chat_id} ({first_name})")


# 🔄 Проверка матчей в фоне
...
# 🔁 Запускаем два процесса — Telegram и анализ матчей
async def main():
